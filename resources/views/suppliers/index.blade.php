...
@section('content')
<!-- Custom UI Styling Matching Exact Image -->
<style>
    body {
...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div class="d-flex gap-2">
        ...
        <a href="{{ route('suppliers.invite') }}" class="btn btn-invite-supplier d-flex align-items-center gap-2 text-decoration-none">
            <i class="bi bi-reply-fill" style="transform: scaleX(-1); display: inline-block;"></i> Invite Supplier
        </a>
    </div>
</div>
...
<div class="filter-card p-4 mb-4" id="filterBox">
    <form method="GET" action="{{ route('suppliers.index') }}" id="filterForm">
        ...
        <div class="text-end mt-2">
            <a href="{{ route('suppliers.index') }}" class="text-decoration-none clear-btn">Clear All</a>
        </div>
    </form>
</div>
...
<div class="text-muted mb-3 ms-1" style="font-size: 13px;">
    Showing: <span class="fw-bold text-dark">{{ method_exists($suppliers, 'firstItem') ? ($suppliers->firstItem() ?? 0) : count($suppliers) }}</span> of <span class="fw-bold text-dark">{{ method_exists($suppliers, 'total') ? $suppliers->total() : count($suppliers) }}</span>
</div>
                ...
                <tr>
                    ...
                    <td class="text-center position-relative">
                        ...
                        <ul class="custom-dropdown-menu">
                            <li>
                                <a class="custom-dropdown-item" href="{{ route('suppliers.show', $supplier->id) }}">
                                    <i class="bi bi-eye me-2 text-muted"></i> View
                                </a>
                            </li>
                            <li>
                                <button type="button" class="custom-dropdown-item text-danger delete-btn" data-id="{{ $supplier->id }}">
    ...
    <div class="modal-dialog modal-dialog-centered modal-sm">
        <div class="modal-content text-center p-3 border-0 shadow" style="border-radius: 12px;">
            <div class="modal-body">
                ...
                <p class="text-muted" style="font-size: 12px;">You will not be able to recover this Supplier</p>
...
$(document).ready(function() {
    ...
    $(document).on('click', '.delete-btn', function() {
        $('.custom-dropdown-menu').removeClass('active');
        let id = $(this).data('id');
        let url = "{{ route('suppliers.destroy', ':id') }}".replace(':id', id);
