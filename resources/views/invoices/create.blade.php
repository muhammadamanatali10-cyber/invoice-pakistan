...
<form action="{{ route('invoices.store') }}" method="POST" id="invoiceForm">
    ...
    <div class="d-flex justify-content-between align-items-center mb-1">
        <h3 class="page-title mb-0">New Invoice</h3>
    ...
    <nav aria-label="breadcrumb" class="mb-4">
        <ol class="breadcrumb mb-0" style="font-size: 13px;">
            ...
            <li class="breadcrumb-item"><a href="{{ route('invoices.index') }}" class="text-decoration-none text-muted">Invoices</a></li>
            <li class="breadcrumb-item active text-muted" aria-current="page">New Invoice</li>
        </ol>
    </nav>
        ...
        <div class="row g-4">
            ...
            <div class="col-md-5">
                <div class="position-relative">
                    <div class="customer-card-box" id="customerBox">
                        @php
                            $selectedCustomer = isset($estimate) ? $estimate->customer : null;
                        @endphp
                        <div id="customerSelectedView" class="{{ $selectedCustomer ? 'd-flex' : 'd-none' }} align-items-center gap-3">
                            <div class="avatar-circle" id="selectedAvatar">{{ $selectedCustomer ? strtoupper(substr($selectedCustomer->name, 0, 1)) : 'A' }}</div>
                            <div>
                                <h6 class="fw-bold mb-0 text-dark" id="selectedName">{{ $selectedCustomer->name ?? '' }}</h6>
                            </div>
                        </div>
                        <div id="customerPlaceholder" class="text-center py-3 {{ $selectedCustomer ? 'd-none' : '' }}">
                        ...
                        <input type="hidden" name="customer_id" id="customerId" value="{{ $selectedCustomer->id ?? '' }}" required>
                    </div>
            ...
            <div class="col-md-7">
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        <label class="form-label-custom">Invoice Number <span class="text-danger">*</span></label>
                        <div class="input-icon-group">
                            <i class="bi bi-hash"></i>
                            <input type="text" name="invoice_number" class="form-control bg-light" value="{{ $nextInvoiceNumber ?? 'INV - ' . str_pad((\App\Models\Invoice::count() + 1), 6, '0', STR_PAD_LEFT) }}" readonly>
                        </div>
                    </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="input-icon-group">
                            <i class="bi bi-hash"></i>
                            <input type="text" name="ref_number" class="form-control" value="{{ $estimate->ref_number ?? '' }}" placeholder="#">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <table class="table align-middle mb-0">
                ...
                <tbody id="itemRows">
                    @if(isset($estimate) && $estimate->items->count() > 0)
                        @foreach($estimate->items as $index => $estItem)
                        <tr>
                            <td class="text-muted"><i class="bi bi-grid-3x2-gap-fill" style="cursor: grab; color: #cbd5e1;"></i></td>
                            <td>
                                <input type="text" name="items[{{ $index }}][name]" class="form-control form-control-sm mb-2 border-0 bg-light p-2 item-name-input" value="{{ $estItem->item_name }}" placeholder="Type or select item name" required style="border-radius: 6px; font-size: 13px;">
                                <input type="text" name="items[{{ $index }}][description]" class="form-control form-control-sm border-0 bg-transparent text-muted p-0 item-desc" value="{{ $estItem->description }}" placeholder="Type Item Description (optional)" style="font-size: 12px;">
                            </td>
                            <td>
                                <input type="number" name="items[{{ $index }}][qty]" class="form-control text-center qty" value="{{ $estItem->quantity }}" min="1" style="border-radius: 6px; border: 1px solid #e2e8f0; height: 38px;">
                            </td>
                            <td>
                                <div class="position-relative">
                                    <span class="price-prefix">Rs</span>
                                    <input type="number" step="0.01" name="items[{{ $index }}][price]" class="form-control price price-input" value="{{ number_format($estItem->price, 2, '.', '') }}" style="border-radius: 6px; border: 1px solid #e2e8f0; height: 38px;">
                                </div>
                            </td>
                            <td class="text-end fw-semibold text-dark" style="font-size: 14px;">
                                Rs <span class="row-amount">{{ number_format($estItem->amount, 2, '.', '') }}</span>
                            </td>
                    ...
                    @else
                        <tr>
                            <td class="text-muted"><i class="bi bi-grid-3x2-gap-fill" style="cursor: grab; color: #cbd5e1;"></i></td>
                            <td>
                                <select name="items[0][name]" class="form-select item-select mb-2 border-0 bg-light p-2" style="border-radius: 6px; font-size: 13px;">
                                    <option value="" selected disabled>Type or click to select an item</option>
                                    @foreach($items as $item)
                                        <option value="{{ $item->name }}" data-price="{{ $item->price ?? 0 }}" data-description="{{ $item->description ?? '' }}">
                                            {{ $item->name }}
                                        </option>
                                    @endforeach
                                </select>
    ...
    <div class="row g-4">
        <div class="col-md-7">
            <div class="card card-custom p-3 mb-3">
                ...
                <textarea name="notes" class="form-control border-0 p-0 shadow-none text-muted" rows="3" placeholder="Enter notes..." style="resize: none; font-size: 13px;">{{ $estimate->notes ?? '' }}</textarea>
            </div>
            ...
            <div class="card card-custom p-4">
                ...
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <div class="d-flex gap-1 align-items-center" style="width: 140px;">
                        <input type="number" id="discountInput" name="discount" class="form-control text-center" value="{{ $estimate->discount ?? 0 }}" style="border-radius: 6px; border: 1px solid #e2e8f0; height: 34px;">
                        <select class="form-select" id="discountType" name="discount_type" style="border-radius: 6px; border: 1px solid #e2e8f0; height: 34px; width: 65px;">
                            <option value="flat" {{ (isset($estimate) && $estimate->discount_type == 'flat') ? 'selected' : '' }}>Rs</option>
                            <option value="percent" {{ (isset($estimate) && $estimate->discount_type == 'percent') ? 'selected' : '' }}>%</option>
                        </select>
                    </div>
                </div>
...
document.addEventListener('DOMContentLoaded', function () {
    let rowIndex = {{ isset($estimate) ? $estimate->items->count() : 1 }};
    ...
    document.getElementById('addRowBtn').addEventListener('click', function () {
        ...
        itemsData.forEach(item => {
            options += `<option value="${item.name}" data-price="${item.price || 0}" data-description="${item.description || ''}">${item.name}</option>`;
        });

        tr.innerHTML = `
            <td class="text-muted"><i class="bi bi-grid-3x2-gap-fill" style="cursor: grab; color: #cbd5e1;"></i></td>
            <td>
                <select name="items[${rowIndex}][name]" class="form-select item-select mb-2 border-0 bg-light p-2" style="border-radius: 6px; font-size: 13px;">
                    ${options}
                </select>
